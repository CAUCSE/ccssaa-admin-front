...
import { useEmailCampaign, useEmailCampaignRecipients, useSendEmailCampaign } from "@/hooks/useEmailCampaigns"
...
function CampaignDetail({ id }: { id: string }) {
  ...
  const [page, setPage] = useState(1); const [status, setStatus] = useState<EmailCampaignRecipientStatus | undefined>(); const [confirmOpen, setConfirmOpen] = useState(false); const [confirmedSubject, setConfirmedSubject] = useState(""); const [confirmedCount, setConfirmedCount] = useState("")
  ...
  return <div className="space-y-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><PageHeader title={campaign.subject} description="서버 정제본과 수신자별 발송 결과를 확인합니다." breadcrumbs={[{ label: "이메일 캠페인", href: "/admin/email-campaigns" }, { label: "상세" }]}/><Button disabled={!canSend} onClick={() => { setConfirmedSubject(""); setConfirmedCount(""); setConfirmOpen(true) }}>발송 요청</Button></div>
    ...
    <Card><CardHeader><CardTitle>정제된 이메일 본문</CardTitle></CardHeader><CardContent><div className="rounded-md border bg-white p-6 text-slate-950 [&_a]:text-blue-600 [&_img]:max-w-full [&_table]:w-full [&_table]:border-collapse [&_td]:border [&_td]:p-2 [&_th]:border [&_th]:p-2" dangerouslySetInnerHTML={{ __html: campaign.sanitizedHtml ?? "" }}/><p className="mt-4 text-xs text-muted-foreground">생성 {formatDateTime(campaign.createdAt)}{campaign.completedAt ? ` · 완료 ${formatDateTime(campaign.completedAt)}` : ""}</p></CardContent></Card>
